...
#
# Writes: dist/colonywarsredsun-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix colonywarsredsun \
  --exe-name Colony_Wars_Red_Sun \
  --omit-openbios \
  --display-name "Colony Wars: Red Sun" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned Colony Wars: Red Sun (USA) disc" \
  --runtime-dir mods \
